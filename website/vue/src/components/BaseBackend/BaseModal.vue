            ...
            >
              ...
              >
                <el-upload
                  ...
                  :action="item.action ? item.action : api + '/file/uploadFile'"
                  :name="item.name || 'file'"
                  :data="item.params || {}"
                  :list-type="item.listType ? item.listType : 'text'"
                  :on-success="
              ...
              >
                <div class="row">
                  <component
                    ...
                    v-bind="item"
                    :disabled="type == 'detail' || item.disabled"
                    @after-opreate="componentAfterOperate(item)"
                  ></component>
                  <span v-if="item.after" class="ml5 w50px">{{
                    item.after
                  }}</span>
...
<script>
import { env as api } from '@/network/request.js'
...
export default {
  ...
  data () {
    const formData = {}
    const items = (this.$props.items || []).map((unit, index) => {
      if (!unit.key) {
        unit.key = `key${index}`
      }
      return unit
    ...
    items.forEach((item) => {
      if (
        ...
        formData[item.key] = {}
        formData[item.key].start = item.data ? item.data.start || null : null
        formData[item.key].end = item.data ? item.data.end || null : null
  ...
  methods: {
    ...
    fileExceed () {
      this.$message.warning('所选文件不超过5个')
    },
    handleChange (item, file, fileList) {
    ...
    componentAfterOperate (item) {
      item.method && item.method(this.formData, this.formData[item.key])
    },
    beforeUpload (file) {
      this.openLoading()
    ...
    genDebugData () {
      for (const index in this.items) {
        ...
        if (item.dataList && item.dataList.length) {
          this.formData[key] = { value: item.dataList[0].value }
        } else if (
          item.type === 'slot' ||
          item.type === 'uploadFile' ||
          item.type === 'autoupload' ||
          ...
          if (!item.dateType) {
            this.formData[key] = moment().format('YYYY-MM-DD')
          }
        } else {
    ...
    ok () {
      ...
      if (isValidPass) {
        ...
        if (this.type === 'add') {
          this.$emit('add', result, this.params)
        } else if (this.type === 'edit') {
          this.$emit('edit', result, this.params)
        }
      } else {
        this.$emit('error', errMsgList)
      }
    },
    cancel () {
      this.$emit('cancel')
